...
class _EditCourseScreenState extends State<EditCourseScreen> {
  ...
  void initState() {
    ...
    _courseCodeController = TextEditingController(
      text: (widget.courseData['code'] ?? '').toString(),
    );

    _courseNameController = TextEditingController(
      text: (widget.courseData['name'] ?? '').toString(),
    );

    _teacherController = TextEditingController(
      text: (widget.courseData['teacher'] ?? '').toString(),
    );
    ...
    _durationController = TextEditingController(
      text: (widget.courseData['duration'] ?? '').toString(),
    );

    _descriptionController = TextEditingController(
      text: (widget.courseData['description'] ?? '').toString(),
    );

    _level = widget.courseData['level'];
    _studyTime = widget.courseData['studyTime'];

    if (_level != null && !_levelList.contains(_level)) {
      _level = null;
    }

    if (_studyTime != null && !_studyTimeList.contains(_studyTime)) {
      _studyTime = null;
    }
  }
  ...
  // todo: CHECK COURSE CODE (excluding current course)
  Future<bool> _isCourseCodeTaken(String code) async {
    final query = await FirebaseFirestore.instance
        .collection('courses')
        .where('code', isEqualTo: code)
        .get();

    return query.docs.any((doc) => doc.id != widget.courseId);
  }
  ...
  Future<void> _updateCourse() async {
    ...
    try {
      ...
      final taken = await _isCourseCodeTaken(courseCode);
      ...
      _showMessage('Failed to update course: ${e.toString()}');
    }
  }
